...
function handleFileUpload($fileInputName, $targetDir = "../assets/images/")
{
    if (isset($_FILES[$fileInputName]) && $_FILES[$fileInputName]['error'] === UPLOAD_ERR_OK) {
        ...
        if (move_uploaded_file($_FILES[$fileInputName]["tmp_name"], $targetFile)) {
            return str_replace('../', '', $targetFile); // Store relative to root
        }
    }
    return null;
}
        ...
        for ($i = 0; $i < count($galleryInputs['name']); $i++) {
            if ($galleryInputs['error'][$i] === UPLOAD_ERR_OK) {
                ...
                if (move_uploaded_file($tmpName, $targetPath)) {
                    ...
                    $conn->query("INSERT INTO hotel_images (hotel_id, image_path) VALUES ($id, '$dbPath')");
                }
            }
        }
    }
...
<head>
    <meta charset="UTF-8">
    <title>Edit Hotel - Travolo Admin</title>
    ...
    <style>
        ...
        /* Sidebar Styles (Mirrored from admin.php) */
        .sidebar {
            width: 260px;
...
<body>
    ...
    <div class="sidebar">
        ...
        <div class="sidebar-menu">
            <a href="admin.php"><i class="fas fa-plane-departure"></i> Flight Bookings</a>
            <a href="admin.php?tab=cabs"><i class="fas fa-car-side"></i> Cab Bookings</a>
            <a href="admin.php?tab=hotels"><i class="fas fa-hotel"></i> Hotel Bookings</a>
            <a href="admin.php?tab=manage-hotels" class="active"><i class="fas fa-building"></i> Manage Hotels</a>
            <a href="admin.php?tab=manage-offers"><i class="fas fa-tags"></i> Manage Offers</a>
            <a href="admin.php?tab=manage-cabs"><i class="fas fa-taxi"></i> Manage Cabs</a>
            <a href="admin.php?tab=manage-routes"><i class="fas fa-route"></i> Manage Routes</a>
            <a href="admin.php?tab=contacts"><i class="fas fa-envelope-open-text"></i> Messages</a>
        </div>
    ...
    <div class="main-content">
        <div class="container-fluid">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <a href="admin.php" class="btn btn-outline-secondary rounded-pill px-4"><i
                    ...
                    <div class="row g-4">
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="number" name="price" class="form-control"
                                value="<?php echo $hotel['price']; ?>" required>
                        </div>
                        <div class="col-md-4">
                            ...
                            <select class="form-select form-control" name="accommodations" required>
                                ...
                                foreach ($types as $type)
                                    echo "<option value='$type' " . ($hotel['accommodations'] == $type ? 'selected' : '') . ">$type</option>"; ?>
                            </select>
                        </div>
                        <div class="col-md-4">
                        ...
                        <div class="col-12">
                            ...
                            <div class="row g-2 p-3 bg-light rounded shadow-sm border">
                                ...
                                while ($img = $gi->fetch_assoc()) {
                                    $displayPath = strpos($img['image_path'], 'assets/') === 0 ? '../' . $img['image_path'] : $img['image_path'];
                                    echo "<div class='col-md-1 col-sm-3 position-relative'>
                                        <img src='{$displayPath}' class='rounded-3 w-100' style='height:60px; object-fit:cover;'>
            ...
            <div class="edit-card shadow-lg" style="border: 1px solid #ffe8cc;">
                        ...
                        <tbody>
                            ...
                            if ($rooms->num_rows > 0):
                                while ($rm = $rooms->fetch_assoc()):
                                    ...
                                    $roomImgPath = strpos($rm['room_image'], 'assets/') === 0 ? '../' . $rm['room_image'] : $rm['room_image'];
                                    ?>
                                    <tr>
                                        <td><img src="<?php echo $roomImgPath; ?>"></td>
                                ...
                                <?php endwhile; else: ?>
                                <tr>
                                    <td colspan="7" class="text-center py-4 text-muted">No rooms added yet. Add one below!
                                    </td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
                ...
                <div class="bg-light p-4 rounded-4 border border-warning border-opacity-25">
                    <h5 class="fw-bold mb-4 text-warning"><i class="fas fa-plus-circle me-2"></i>Add New Room Variety
                    </h5>
                    <form method="POST" enctype="multipart/form-data">
                        ...
                        <div class="row g-3">
                            <div class="col-md-4">
                                ...
                                <input type="text" name="room_name" class="form-control" placeholder="e.g. Deluxe Suite"
                                    required>
                            </div>
                            <div class="col-md-2">
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="d-flex flex-wrap gap-2 mt-1">
                                    <label class="btn btn-sm btn-outline-primary"><input type="checkbox"
                                            name="features[]" value="AC" hidden> AC</label>
                                    <label class="btn btn-sm btn-outline-primary"><input type="checkbox"
                                            name="features[]" value="Free WiFi" hidden> WiFi</label>
                                    <label class="btn btn-sm btn-outline-primary"><input type="checkbox"
                                            name="features[]" value="Bathtub" hidden> Bathtub</label>
                                    <label class="btn btn-sm btn-outline-primary"><input type="checkbox"
                                            name="features[]" value="Minibar" hidden> Minibar</label>
                                    <label class="btn btn-sm btn-outline-primary"><input type="checkbox"
                                            name="features[]" value="City View" hidden> View</label>
                                </div>
                            </div>
                            <div class="col-12 mt-3">
                                <button type="submit"
                                    class="btn btn-warning w-100 fw-bold py-2 rounded-3 text-white">ADD ROOM TO
                                    SYSTEM</button>
                            </div>
                        </div>
                    </form>
