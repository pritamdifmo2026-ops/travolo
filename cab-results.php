...
<head>
    ...
    <style>
        ...
        header, .navbar, .header-navigation, .nav-menu, .nav-overlay {
    ...
    <div class="results-header-box" style="background: linear-gradient(90deg, #1aa39a, #158b83); padding: 15px 0;">
        <div class="container">
            <form action="cab-results.php" method="GET" id="inlineSearchForm" class="row g-2 align-items-end">
                ...
                <div class="col-12 text-white mb-2 d-flex flex-wrap gap-4 align-items-center"
                    style="font-size: 12px; font-weight: 700;">
                    <label class="d-flex align-items-center gap-1 cursor-pointer" style="cursor: pointer;">
                        <input type="radio" name="tripType" value="Transfer" <?php if ($tripType === 'Transfer' || $tripType === 'Airport Transfer')
                            echo 'checked'; ?>
                            onchange="document.getElementById('inlineSearchForm').submit();">
                        AIRPORT TRANSFER
                    </label>
                    <label class="d-flex align-items-center gap-1 cursor-pointer" style="cursor: pointer;">
                        <input type="radio" name="tripType" value="Outstation" <?php if ($tripType === 'Outstation')
                            echo 'checked'; ?> onchange="document.getElementById('inlineSearchForm').submit();">
                        OUTSTATION/OTHER
                    </label>
                    <label class="d-flex align-items-center gap-1 cursor-pointer" style="cursor: pointer;">
                        <input type="radio" name="tripType" value="Hourly" <?php if ($tripType === 'Hourly')
                            echo 'checked'; ?> onchange="document.getElementById('inlineSearchForm').submit();">
                        HOURLY
                    </label>
                </div>
                ...
                <?php if ($tripType === 'Hourly'): ?>
                    <div class="col-md-5">
                        <input type="text" name="from" class="form-control rounded-1 border-0 py-2"
                            value="<?php echo htmlspecialchars($from); ?>" placeholder="Enter Pick-up Location" required>
                    </div>
                    <div class="col-md-3">
                        <label class="text-white small fw-bold mb-1 d-block" style="font-size: 10px;">PICK-UP DATE &
                            TIME</label>
                        <div class="d-flex gap-1">
                            <input type="date" name="date" class="form-control rounded-1 border-0 py-2 px-2"
                                value="<?php echo htmlspecialchars($date); ?>" required>
                            <input type="time" name="time" class="form-control rounded-1 border-0 py-2 px-2"
                    ...
                    <div class="col-md-2">
                        ...
                        <style>
                            ...
                            #durationForm .nice-select {
                                display: none !important;
                            }
                        </style>
                        <select name="duration"
                            class="form-select rounded-1 border-0 py-2 fw-bold text-dark ignore-nice-select"
                    ...
                    <div class="col-md-2 text-end">
                        <button type="submit" class="btn rounded-pill text-white fw-bold px-4 py-2 border border-white"
                            style="background: transparent; margin-top: 15px;">SEARCH</button>
                    </div>
                <?php else: ?>
                    <div class="col-md-3">
                        <label class="text-white small fw-bold mb-1 d-block" style="font-size: 10px;">FROM</label>
                        <input type="text" name="from" class="form-control rounded-1 border-0 py-2"
                            value="<?php echo htmlspecialchars($from); ?>" placeholder="Pick-up City/Airport" required>
                    </div>
                    <div class="col-md-3">
                        <label class="text-white small fw-bold mb-1 d-block" style="font-size: 10px;">TO</label>
                        <input type="text" name="to" class="form-control rounded-1 border-0 py-2"
                            value="<?php echo htmlspecialchars($to); ?>" placeholder="Drop City/Airport" required>
                    </div>
                    <div class="col-md-4">
                        <label class="text-white small fw-bold mb-1 d-block" style="font-size: 10px;">PICK-UP DATE &
                            TIME</label>
        ...
        <div class="row">
            ...
            <div class="col-lg-3 d-none d-lg-block">
                <div class="checkout-box p-4"
                    style="background: #fff; border-radius: 16px; border: 1px solid #eee; position: sticky; top: 100px;">
                    <div class="d-flex justify-content-between align-items-center mb-4">
                        <h6 class="fw-bold mb-0">Filters</h6>
                        <a href="cab-results.php?from=<?php echo urlencode($from); ?>&to=<?php echo urlencode($to); ?>&tripType=<?php echo urlencode($tripType); ?>"
                            class="text-primary small text-decoration-none">Reset</a>
                    </div>
                    ...
                    <div class="filter-section">
                        ...
                        <div class="form-check mb-2">
                            <input class="form-check-input" type="checkbox" value="CNG" id="fuelCNG"
                                onchange="filterCabs()">
            ...
            <div class="col-lg-9">
                <div class="mb-4 d-flex align-items-center justify-content-between">
                    ...
                    <div>
                        ...
                        <p class="text-muted small mb-0"><i class="fas fa-info-circle me-1"></i> All prices include GST
                            and estimated Tolls</p>
                    </div>
                    <div class="d-flex gap-2">
                        <span class="badge bg-white text-dark border p-2 small fw-normal"><i
                                class="fas fa-sort-amount-down me-1"></i> Price: Low to High</span>
                    </div>
                </div>
                            ...
                            while ($cab = $cabs_res->fetch_assoc()) {
                                ...
                                <div class="car-result-card cab-item" data-category="<?php echo $cab['category']; ?>"
                                    ...
                                    <div class="car-detail-main">
                                        <span
                                            class="car-category <?php echo $cat_class; ?>"><?php echo htmlspecialchars($cab['category']); ?></span>
                                        <h3 class="car-name-title mb-1"><?php echo htmlspecialchars($cab['car_name']); ?> <small
                                                class="text-muted fw-normal" style="font-size: 14px;">or Equivalent</small></h3>

                                        <div class="car-features-icons">
                                            <div class="feature-icon-item"><i class="fas fa-users"></i>
                                                <?php echo $cab['capacity']; ?> Seats</div>
                                            <div class="feature-icon-item"><i class="fas fa-briefcase"></i>
                                                <?php echo $cab['luggage']; ?> Bags</div>
                                            <div class="feature-icon-item"><i class="fas fa-snowflake"></i> AC</div>
                                            <div class="feature-icon-item"><i class="fas fa-gas-pump"></i> <?php echo $fuel; ?>
                                            </div>
                                        </div>
                                        ...
                                        <div class="mt-3">
                                            <div class="d-flex align-items-center gap-2 mb-2">
                                                <span
                                                    class="badge bg-success-subtle text-success small border border-success-subtle px-2"><i
                                                        class="fas fa-check me-1"></i> Free Cancellation</span>
                                                <span
                                                    class="badge bg-primary-subtle text-primary small border border-primary-subtle px-2"><i
                                                        class="fas fa-bolt me-1"></i> Instant Confirmation</span>
                                            </div>
                                            <p class="small text-muted mb-0"><i class="fas fa-shield-alt text-warning me-1"></i>
                                                Safety Guaranteed Chauffeurs</p>
                                        </div>
                                    </div>
                                    <div class="car-price-section">
                                        <div class="text-muted small mb-0 fw-bold"
